...
class LocalGemmaChatService {
  ...
  String _buildPrompt(
    ...
    final lines = <String>[
      ...
      '- 후보 저장: 카메라 매칭 화면에서 매장 거울이나 옷을 비춘 상태로 후보를 저장한다. 저장 후보는 최대 3개까지 비교한다.',
      '- 후보 비교: 후보가 2개 이상이면 "비교하기"에서 AI 비교를 실행한다. 처리 중에는 후보 선택/삭제/상세 이동을 기다린다.',
      '- 후보 상세: 저장 후보를 열면 이미지, 메모, AI 분석 결과를 볼 수 있다. 필요하면 메모를 수정한다.',
      '- 퍼스널 컬러: 설정의 "퍼스널 컬러 확인"에서 현재 등록된 얼굴 이미지 기준으로 추천/피해야 할 색을 확인한다.',
      '- AI 엔진 설정: 설정의 "AI 설정"에서 꺼짐, Mock, Local Gemma, OpenAI 중 선택한다.',
      ...
      '- OpenAI 사용: AI 엔진을 OpenAI로 선택하고 OpenAI 프록시 주소를 저장한 뒤 연결 테스트를 통과해야 한다. API key는 앱이 아니라 프록시 서버에 둔다.',
      '- 처리 시간: Local Gemma는 휴대폰에서 직접 실행되므로 응답이나 후보 비교가 1분 이상 걸릴 수 있다. 처리 중에는 앱을 닫지 않는 것이 좋다.',
    ];
